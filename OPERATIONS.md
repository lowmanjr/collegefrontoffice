...
**Fix implemented April 2026:** Added `search_espn_player_relaxed(name, position)` as a Pass 3 fallback applied only when `acquisition_type = 'portal'`. Safeguards: exact normalized name match required (no fuzzy), position match enforced when ESPN provides one, skip if multiple ESPN results match the same name+position (ambiguous). Logs relaxed matches separately as `[PORTAL-RELAXED]` with both DB and ESPN team names. Non-portal players retain the strict team-nickname verification to avoid false positives on common names.
