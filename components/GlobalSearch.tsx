...
export default function GlobalSearch() {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {/* ── Input ──────────────────────────────────────────────────────── */}
      <div className="flex items-center gap-2 bg-slate-800 border border-slate-700 rounded-lg px-3 py-1.5 w-48 sm:w-64 focus-within:border-slate-500 transition-colors">
        ...
        {isSearching && (
          <svg
            ...
            aria-hidden="true"
          >
            <circle cx="6" cy="6" r="5" stroke="currentColor" strokeWidth="2" strokeDasharray="8 24" strokeLinecap="round" />
          </svg>
        )}
      </div>
      ...
      {isOpen && hasResults && (
        ...
        >
          ...
          <p className="px-4 py-2 text-[10px] text-slate-400 border-t border-gray-100 bg-slate-50">
            Press <kbd className="rounded bg-slate-200 px-1 py-0.5 font-mono text-slate-600">Esc</kbd> to close
          </p>
