"""
Scrape On3 basketball transfer portal (committed players) to update roster assignments.
Source: https://www.on3.com/transfer-portal/wire/basketball/?status=committed
...
Usage: python sync_basketball_transfer_portal.py [--dry-run] [--max-pages 20]
"""
...
def scrape_portal_page(page_num: int) -> list[dict]:
    """Scrape one page of On3 committed basketball transfers."""
    url = f"https://www.on3.com/transfer-portal/wire/basketball/?status=committed&page={page_num}"
    try:
...
def main():
    ...
    max_pages = 20  # basketball portal is much smaller than football
    for i, arg in enumerate(sys.argv):
        if arg == "--max-pages" and i + 1 < len(sys.argv):
            max_pages = int(sys.argv[i + 1])

    # Fetch our teams
    teams_resp = supabase.table("basketball_teams").select("id, university_name").execute()
    ...
    while True:
        ...
        if len(batch) < page_size:
            break
        offset += page_size
    log.info(f"Found {len(all_db_players)} basketball players in DB")
    ...
    # Scrape On3 portal
    log.info(f"Scraping On3 basketball transfer portal (committed, up to {max_pages} pages)...")
    all_transfers = []
    ...
    for transfer in all_transfers:
        ...
        if db_player:
            ...
            if dry_run:
                ...
                try:
                    supabase.table("basketball_players").insert({
                        "name": transfer["name"],
