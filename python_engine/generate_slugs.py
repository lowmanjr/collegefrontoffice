"""
Generate URL slugs for all teams and players.
Handles duplicate names by appending team name for players.

Usage: python generate_slugs.py [--dry-run]
"""
...
def main():
    ...
    # ── Teams ────────────────────────────────────────────────────────────
    log.info("Generating team slugs...")
    teams_resp = supabase.table("teams").select("id, university_name, slug").execute()
    ...
    for t in teams:
        ...
        if dry_run:
            log.info(f"  [DRY RUN] {t['university_name']} -> /teams/{slug}")
        else:
            supabase.table("teams").update({"slug": slug}).eq("id", t["id"]).execute()
        team_updates += 1
    ...
    for base, players in base_slugs.items():
        if len(players) == 1:
            ...
            # Duplicate name — append team slug
            for p in players:
                ...
                # Still could collide (same name, same team — unlikely but handle it)
                if slug in slug_set:
                    slug = f"{slug}-{p['id'][:8]}"
